...
export const BranchSelector: React.FC<BranchSelectorProps> = ({ onSelect, isDarkMode, setIsDarkMode, language, setLanguage }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[200] flex items-start sm:items-center justify-center p-4 pt-16 sm:pt-4 overflow-y-auto ${isDarkMode ? 'bg-charcoal-950' : 'bg-cream-100'}`}>
      {/* Background texture */}
      ...
      <div className="relative w-full max-w-4xl">
        ...
        <div className="text-center mb-6 sm:mb-10">
          <div className="flex flex-col items-center gap-2 sm:gap-3">
            <div className="w-24 sm:w-32 md:w-40 flex items-center justify-center">
              <img src="/logo1.png" alt="Pasa Pizzeria" className="w-full h-auto object-contain" />
            </div>
          </div>
          <p className={`text-sm sm:text-base md:text-lg font-light mt-3 sm:mt-5 ${isDarkMode ? 'text-cream-400' : 'text-charcoal-500'}`}>
          ...
          <div className="flex items-center justify-center gap-3 mt-3 sm:mt-4">
            <div className={`inline-flex items-center gap-1 px-2 py-1.5 rounded-full ${isDarkMode ? 'bg-charcoal-800' : 'bg-cream-200/60'} text-xs font-bold`}>
            ...
            <button
              ...
              className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${isDarkMode ? 'bg-charcoal-800 text-cream-300 hover:bg-charcoal-700' : 'bg-cream-200/60 text-charcoal-600 hover:bg-cream-300'}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-5 md:gap-8">
          {branches.map((branch) => (
            <button
              ...
              className={`group relative overflow-hidden rounded-3xl p-5 sm:p-8 md:p-10 text-center transition-all duration-300 card-lift border ${
                isDarkMode
            ...
            >
              ...
              <div className="relative">
                <div className="flex justify-center mb-3 sm:mb-5 transform group-hover:scale-110 transition-transform duration-300">
                  {branch.id === 'syria' ? <SyriaFlag /> : <TurkeyFlag />}
                </div>

                <h2 className={`font-serif text-xl sm:text-2xl md:text-3xl font-bold mb-1 sm:mb-2 ${isDarkMode ? 'text-cream-100' : 'text-charcoal-900'}`}>
                  {language === 'ar' && branch.nameAr ? branch.nameAr : (language === 'tr' && branch.nameTr ? branch.nameTr : branch.name)}
                </h2>

                <div className={`mt-4 sm:mt-6 inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-full text-sm font-bold transition-all duration-300 ${
                  isDarkMode
        ...
        {/* Footer hint */}
        <p className={`text-center mt-6 sm:mt-10 text-xs font-medium ${isDarkMode ? 'text-cream-600' : 'text-charcoal-400'}`}>
          {t('يمكنك تبديل الفروع في أي وقت من قائمة الرأس', 'Şubeleri istediğiniz zaman üst menüden değiştirebilirsiniz', 'You can switch branches anytime from the header menu')}
        </p>
      </div>
    </div>
  );
